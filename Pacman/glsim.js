...
const SimGL = (function () {
  ...
  async function init(cv, netSize) {
    ...
    ['sim', 'action', 'reduce', 'com', 'crop', 'draw', 'eatreduce', 'eatsum'].forEach(n => prog[n] = link(src.vertex, src[n], n));
  ...
  function setBoard(w, h) {
    ...
    ringIdx = RING - 1;
    // Zero-filled, so the draw pass has something to sample before the first uploadState2().
    ch2Tex = []; ch2Fbo = [];
    const empty = new Float32Array(W * H);
    for (let i = 0; i < 2; i++) {
      const t = tex(gl.R32F, gl.RED, gl.FLOAT, W, H, empty);
      ch2Tex.push(t); ch2Fbo.push(fbo([t]));
    }
    ch2Idx = 0;
  ...
  function runSim(srcTex, dstFbo, kern, R, mu, sigma, dt, eat, wallEnabled) {
  ...
  function step(action) {
    ...
    runSim(input, ringFbo[dst], kernelTex, kR, ruleMu, ruleSigma, ruleDt, null, true);
    ringIdx = dst;
  ...
  return { init, setBoard, setWall, setRule, setRule2, uploadState, uploadState2, prime, step, readback, draw };
})();
